...
def discretize_state(history: jnp.ndarray, n_bins: int = 10) -> int:
    if history.size == 0:
        return 0
    mean_cp = jnp.mean(history)
    discretized = jnp.floor(mean_cp * n_bins).astype(jnp.int32)
    return jnp.clip(discretized, 0, n_bins - 1)

# def current_group(snr: float) -> int:
#     # Kwantyzacja SNR na indeksy 0, 1, 2
#     if 40 <= snr <= 70:
#         snr_index = 0
#     elif 30 <= snr < 40:
#         snr_index = 1
...
def current_group(snr: float) -> int:
    ...
    if snr < 12:
        return 0  # Skrajnie niski SNR (poza zakresem działania MCS 0-11)
    elif snr >= 63:
        return 25  # Skrajnie wysoki SNR (powyżej MCS 11)
    else:
        # Podział co 1 dB, każdy przedział to jeden stan → 52 grupy
        return int((snr - 12) // 1)
...
class IEEE_802_11_ax_RA(BaseExt):
    ...
    def env_state(
        ...
        return jnp.array([self.delta_time(time), n_successful, n_failed, n_wifi, power, cw, snr], dtype=float)

    @observation(observation_type=gym.spaces.Discrete(3))
    def current_state(self, snr: float, *args, **kwargs) -> int:
        return current_group(snr)
